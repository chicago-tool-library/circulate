module Neon
  ...
  def self.account_to_synced_attributes(account)
    ...
    lookup = ->(account, key) {
      account.dig(*key.split(".").map { |v| v =~ /^\d$/ ? v.to_i : v })
    }

    FIELD_MAP.inject({}) { |sum, (key, definition)|
      if definition.is_a?(Hash)
        if lookup_key = definition[:key]
          value = lookup.(account, lookup_key)
        else
          value = customFields.find { |f| f["name"] == definition[:name] }.dig("value")
        end
        value = definition[:finalize].(value) if definition.key?(:finalize)
        sum[key.to_s] = value
      elsif definition.is_a?(Proc)
        sum[key.to_s] = definition.(account)
      else
        sum[key.to_s] = lookup.(account, definition)
      end
      sum
    }
  end
